...
void muteAudio ( void )
{
    ...
    BSP_CODEC_AK4384_AMPLIFIER_POWER_DOWN_PIN_CLR;  // turn off amplifier
}
/*****************************************************
...
{    
    ...
    {       
        ...
        else    // = 1, therefore will be decremented to 0 before leaving function
        {
            ...
            BSP_CODEC_AK4384_AMPLIFIER_POWER_DOWN_PIN_SET;      // if amp was powred down, power it back up again
        }
...
void APP_TasksCheckStuck()
{
    ...
    { 
        ...
        {
            led3StuckOff++;
            if (led3StuckOff > led5StuckOffMax)
            {
        ...
        case APP_PROCESS_DATA:
            {
                ...
                if(usbNextBuffer == codecNextBuffer)
                {
                    if(bufferCount == 0)
                    {
                        ...
                        codecNextBuffer = 0;                        
                        justOnce = false;
                        // appData.state = APP_SUBMIT_READ_REQUEST; -- removed, want to stay in this state                      
                        usbOverLap++;         
                        /* Initializing appData members*/
                        initData();
                        //break;  -- removed, want to continue remainder of processing in this sate
                    }
                }
...
//
// The MIPS M4K core of the PIC32 has the 32-bit Core Timer. This timer is fed by
// the system clock rather than the peripheral bus clock like all other timers.
// It has a fixed prescaler 1:2.  Therefore for a SYSCLK of 200 MHz, one count of
// the Core Timer is 10 ns.
//
// *****************************************************************************
// *****************************************************************************

uint32_t __attribute__((nomips16)) APP_ReadCoreTimer()
{
...
void __attribute__((nomips16)) APP_StartCoreTimer(uint32_t period)
{
    /* Reset the coutner */
