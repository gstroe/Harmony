...
int32_t DRV_SPI2_SlaveRMSend8BitISR( struct DRV_SPI_OBJ * pDrvObj )
{
    register DRV_SPI_JOB_OBJECT * currentJob = pDrvObj->currentJob;
    if (currentJob == NULL)
    {
    ...
    PLIB_SPI_BufferWrite(SPI_ID_3, currentJob->txBuffer[currentJob->dataTxed]);
    currentJob->dataTxed++;
    currentJob->dataLeftToTx--;
...
int32_t DRV_SPI2_SlaveRMReceive8BitISR( struct DRV_SPI_OBJ * pDrvObj )
{
    register DRV_SPI_JOB_OBJECT * currentJob = pDrvObj->currentJob;
    ...
    if (!PLIB_SPI_ReceiverBufferIsFull(SPI_ID_3))
    {
        return 0;
    }
    ...
    if (currentJob->dataLeftToRx != 0)
    {
        currentJob->rxBuffer[currentJob->dataRxed] = PLIB_SPI_BufferRead(SPI_ID_3);
        currentJob->dataRxed++;
        currentJob->dataLeftToRx--;
    }
    else
